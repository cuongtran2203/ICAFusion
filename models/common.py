...
class C3(nn.Module):
    ...
    def forward(self, x):
        x_xxx = torch.cat((self.m(self.cv1(x)), self.cv2(x)), dim=1)
        # print("C3-----------------",x_xxx.shape)
...
class CSSA(torch.nn.Module):
  ...
  def __init__(self,c1):
    ...
    self.eca_rgb = ECABlock(kernel_size=3, channel_first=c1)
    self.eca_ir = ECABlock(kernel_size=3, channel_first=c1)
    # print("c1 ------------- ",c1)
    self.cs = ChannelSwitching(switching_thresh=0.3)
